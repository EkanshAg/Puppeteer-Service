        ...
        try {
            ...
            page.on('request', request => {
                ...
                if (resType === 'image' || resType === 'font' || resType === 'other' ||
                    reqUrl.match('naukri.com/uba') || reqUrl.match('googletagmanager.com/gtm.js') ||
...
async function closeBrowser(alreadyOpenedbrowser) {
    if (reqCount % maxTabsToOpen == 0) {
...
async function createBrowserInstance() {
    ...
    let newbrowser = await puppeteer.launch({ headless: true, args: ['--no-sandbox', '--ignore-certificate-errors', '--enable-features=NetworkService', '--user-data-dir=/tmp/session-123'] });
